var http = require('http')
  ...
  , url = require('url')
  , debug = require('debug')('gaproxyd')
  , error = require('debug')('gaproxyd');
...
var downloadGaJS = function downloadGaJs () {
  ...
  var gaOriginalFilePath = publicDir + '/analytics-original.js';
  var gaModifiedFilePath = publicDir + '/analytics-modified.js';
  ...
  request('http://google-analytics.com/analytics.js', function (err, res, body) {
    if (err) {
      error('downloadGaJs: %j', err);
      if (!fs.existsSync(gaOriginalFilePath) || !fs.existsSync(gaModifiedFilePath)) {
        throw new Error('http://google-analytics.com/analytics.js cannot be downloaded. Stop.');
      }
    }
